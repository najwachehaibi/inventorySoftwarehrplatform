...
namespace InventoryBeginners.Controllers
{
    ...
    public class BrandController : Controller
    {
        ...
        public IActionResult Create(Brand item)
        {
            ...
            {
                TempData["SuccessMessage"] = "Brand " + item.Name + " Created Successfully";
        ...
        public IActionResult Edit(Brand item)
        {
            ...
            try
            {
                ...
                if (_Repo.IsItemExists(item.Name, item.Id) == true)
                    errMessage = errMessage + "Brand Name " + item.Name + " Already Exists";

                if (errMessage == "")
                {
                    item = _Repo.Edit(item);
                    TempData["SuccessMessage"] = item.Name + ", Brand Saved Successfully";
                    bolret = true;
                }
            }
            catch (Exception ex)
        ...
        public IActionResult Delete(Brand item)
        {
            ...
            TempData["SuccessMessage"] = "Brand " + item.Name + " Deleted Successfully";
            return RedirectToAction(nameof(Index), new { pg = currentPage });
